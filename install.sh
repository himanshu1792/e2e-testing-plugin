...
echo ""
echo "  Install dependencies + browser binaries (the Script Writer also does this for you):"
echo ""
echo -e "${GRAY}    npm install --save-dev @playwright/test dotenv${RESET}"
echo -e "${GRAY}    npx playwright install chromium${RESET}"
echo ""
...
echo ""
echo "  Make sure your playwright.config.ts loads dotenv and runs headless:"
echo -e "${GRAY}    import 'dotenv/config';${RESET}"
echo -e "${GRAY}    use: { baseURL: process.env.APP_URL, headless: true }${RESET}"
echo ""
echo -e "${GRAY}  (Script Writer authors live in a headed MCP browser; Reviewer runs specs headless.)${RESET}"
echo ""
